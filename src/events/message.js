    ...
    async run (msg) {
        ...
        for (let command of bot.commands.array()) {
            ...
            if (command.info.name == cmd || 
                ...
                if ((command.permission.owner || command.permission.memberPermission.filter(permission => !msg.member.hasPermission(permission)).length > 0) && !config.dev.includes(msg.author.id)) return msg.reply(lang["userMissingPermission"][la])
                ...
                //check if enabled
                if (!command.verification.enabled && !config.dev.includes(msg.author.id)) return msg.reply(lang["commandNotEnabled"][la])
                //check cooldown
                ...
                await bot.commands
                    ...
                    .run(
                        msg, //msg
                        args,
                        command.info.name.includes("/") ? rawArgs.slice(command.info.name.split("/").length - 2) : rawArgs, //args
                        command.info.name.split("/").slice(-1)[0] //cmd
                    )
            }
        }
    }
